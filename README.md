...
| Markdown docs | `CONTRACT.md` | Compact data dictionary of the whole contract (see below) |
...
### CONTRACT.md — the Markdown projection

Every `generate` run also writes a `CONTRACT.md` into the output directory: a
compact, deterministic data dictionary of the whole backend — tables (fields,
nullability, `PK`/enum notes, access line), RPC and edge function signatures,
realtime and storage. No prose filler, no timestamps; order follows the
contract, so the same contract always produces byte-identical Markdown. It is
the fastest way to hand your backend's shape to a colleague or an AI.

The optional top-level `summary:` field (a Markdown string) is carried 1:1
into `CONTRACT.md` — the place where a human or an AI maintains the meaning
context the structured sections cannot express:

```yaml
summary: |
  Party-music backend: guests queue songs round-robin and vote
  democratically. See PLAYER_QUEUE_ALGORITHM.md for the queue mechanics.
```
